...
description: "Create a stylish, production-ready Retrieval-Augmented Generation (RAG) chatbot embedded directly into any website/project. Uses OpenAI ChatKit widget + Cohere embeddings + Qdrant Cloud + Neon Serverless Postgres + FastAPI backend. Supports selected-text questioning and full conversation history."
version: "1.0.0"
---

## **Used MCP servers**:
- context7 for documentation search and research and development

# Integrated RAG Chatbot Developer Skill

## Phase 1 : Create UI
- **OpenAI chatkit SDK**: For chatbot ui , note that the chatbot is created inside the `./textbook` folder which is a docusaurus project .

## Phase 2 : Create Backend route
- **OpenAI agents SDK** for chatbot logic , note that this should be setup using the custom config .
```python
import os
